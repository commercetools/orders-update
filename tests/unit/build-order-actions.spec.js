...
test(`buildOrderActions
  should ignore returnItem which does not have a referencing item`, (t) => {
  const item = {
    id: '123',
    quantity: 100,
    lineItemId: '1245',
  ...
  const actions = buildOrderActions.returnItemState(item, undefined)

  t.equal(actions.length, 0, 'actions array should be empty')
  t.end()
})

test(`buildOrderActions
  should return update actions when returnItem was changed`, (t) => {
  const newItem = {
    id: '123',
    shipmentState: 'Returned',
    paymentState: 'Initial',
  ...
  const actions = buildOrderActions.returnItemState(newItem, oldItem)
  t.equal(actions.length, 2, 'there should be two update actions')

  t.deepEqual(actions[0], {
    action: 'setReturnShipmentState',
    returnItemId: '123',
    shipmentState: 'Returned',
  }, 'new shipmentState should be set')

  t.deepEqual(actions[1], {
    action: 'setReturnPaymentState',
    returnItemId: '123',
    paymentState: 'Initial',
  }, 'new paymentState should be set')
